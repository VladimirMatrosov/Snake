...
import com.snake.eat.finish.model.entity.Food
import com.snake.eat.finish.model.entity.SnakePath
...
class SnakeView @JvmOverloads constructor(
    ...
    private var mSnakePaths = ArrayList<SnakePath>()
    private var countPaths = 5
    private var scores = 0
    private var mFood = Food()
    private val mHandler = Handler()
    private val mSnakeRunnable = SnakeRunnable()
    private lateinit var gameView: IGameContract.View
    ...
    fun initSnake(mView: IGameContract.View){
        gameView = mView
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if (mSnakePaths.isEmpty()) {
            for (i in 0 until countPaths) {
                mSnakePaths.add(SnakePath(getStartX() + (countPaths - 1 - i) * getSize(), getStartY()))
                if (i == 0)
    ...
    private fun addPathSnake() {
        ...
        when (lastPath.mDirection) {
            RIGHT_DIRECTION -> {
                val mPath = SnakePath(lastPath.x - getSize(), lastPath.y, lastPath.mDirection, false)
            ...
            BOTTOM_DIRECTION -> {
                val mPath = SnakePath(lastPath.x, lastPath.y - getSize(), lastPath.mDirection, false)
